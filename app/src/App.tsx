...
import { LayoutGrid, Wallet, Plus, Mic, Sparkles, Settings as SettingsIcon, X } from './icons';
import DashboardScreen from './screens/Dashboard';
import AccountsScreen from './screens/Accounts';
import AccountingScreen, { type OnSaved } from './screens/Accounting';
import AdvisorScreen from './screens/Advisor';
import SettingsScreen from './screens/Settings';
import { type DayFlow, type DayTrade, type FlowState, type StockState } from './data/demo';

type Tab = 'dashboard' | 'accounts' | 'record' | 'advisor' | 'settings';
...
function NavHeader({ tab }: { tab: Tab }) {
  ...
  return (
    <div style={{ padding: '6px 18px 12px' }}>
      <div style={{ fontWeight: 700, color: '#18110C', letterSpacing: -0.5, fontSize: '30px' }}>{titles[tab]}</div>
    </div>
  );
}

function TabBar({ tab, setTab, onRecord }: { tab: Tab; setTab: (t: Tab) => void; onRecord: (voice: boolean) => void }) {
  ...
  const tabs = [
    ...
    { id: 'accounts' as const, label: '配置', Icon: Wallet },
...
function RecordSheet({ open, autoVoice, onClose, onSaved }: { open: boolean; autoVoice: boolean; onClose: () => void; onSaved: OnSaved }) {
      ...
      >
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 18px 12px' }}>
          <div>
            ...
            <div style={{ fontSize: 13, color: 'rgba(28,26,24,0.60)', marginTop: 2 }}>{autoVoice ? '說出關鍵字，AI 自動帶入欄位' : '輕點手動填寫 · 點麥克風可語音'}</div>
          </div>
          <button
            onClick={onClose}
            style={{ width: 36, height: 36, borderRadius: 14, flexShrink: 0, background: 'rgba(28,26,24,0.14)', border: 'none', color: 'rgba(45,36,32,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
          >
            <X size={18} />
          </button>
        </div>
...
export default function App() {
  ...
  const [savedFlows, setSavedFlows] = useState<DayFlow[]>([]);
  const [savedTrades, setSavedTrades] = useState<DayTrade[]>([]);
  ...
  const handleSaved: OnSaved = (kind, data) => {
    if (kind === 'flow') {
      ...
      const d = data as StockState;
      const entry: DayTrade = {
        side: d.side,
        code: d.code,
        name: d.name,
  ...
  return (
    ...
    >
      {/* Dynamic island */}
      <div style={{ position: 'absolute', top: 11, left: '50%', transform: 'translateX(-50%)', width: 126, height: 37, borderRadius: 20, background: '#000', zIndex: 50 }} />
      ...
      {tab === 'advisor' ? (
        ...
        <div style={{ position: 'absolute', top: 122, bottom: 0, left: 0, right: 0, overflowY: 'auto', overflowX: 'hidden', paddingBottom: 130 }}>
          {tab === 'dashboard' && <DashboardScreen hideAmounts={hideAmounts} setHideAmounts={setHideAmounts} savedFlows={savedFlows} savedTrades={savedTrades} />}
