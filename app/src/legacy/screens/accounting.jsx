...
function StockForm({ state, update, onSaved, onDelete, recordId, masterData, computedHoldings = [] }) {
  ...
  // sh/pr also drive the fee/tax input placeholders further down.
  ...
  const {
    gross, feeRate: _feeRate, feeDisc: _feeDisc, feeMult, autoFee, fee, feeOverridden,
    taxRate, autoTax, tax, taxOverridden, net,
  } = computeStockTrade({
    side: state.side, shares: state.shares, price: state.price, brokerObj: _brokerObj,
    assetClass: state.assetClass, feeOverride: state.feeOverride, taxOverride: state.taxOverride,
    taxRateMode: state.taxRateMode,
  });
