...
function generateDayData(date) {
  ...
  // Stock trades: only weekdays, 30% chance
  const trades = [];
  if (!isWeekend && rand() < 0.4) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      const m = String(Math.floor(rand() * 60)).padStart(2, '0');
      const trade = { side, code: s.code, name: s.name, shares, price, time: `${h}:${m}`, broker: '券商交割戶' };
      ...
      <div onClick={e => e.stopPropagation()} style={{
        ...
        <div style={{ padding: '0 18px' }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7,1fr)', gap: 4 }}>
            {week.map((w, i) => (
              <div key={w} style={{
                textAlign: 'center', fontSize: 14, padding: '6px 0',
                ...
                <button key={i} disabled={fut} onClick={() => onPick(new Date(year, month, d))}
                  style={{
                    ...
                    color: sel ? '#fff'
                      ...
                      : '#18110C',
                    fontSize: 15, fontWeight: sel ? 700 : td ? 600 : 500,
        ...
        <div style={{ marginTop: 16, padding: '0 18px', display: 'flex', gap: 10 }}>
          <button onClick={() => onPick(new Date(TODAY))} style={{
            ...
            background: 'rgba(28,26,24,0.12)', border: '1px solid rgba(28,26,24,0.14)',
            color: 'rgba(45,36,32,0.75)', fontSize: 15, fontWeight: 500,
          }}>回到今日</button>
...
function DailyView({ date, hideAmounts, extraFlows = [], extraTrades = [] }) {
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div style={{ color: 'rgba(18,17,12,0.72)', fontSize: 14, letterSpacing: 1,
        ...
        {[
          ...
          <div key={x.l} style={{
            ...
            <div style={{ marginTop: 2, fontFamily: 'JetBrains Mono, monospace',
              fontSize: 15, fontWeight: 600, color: x.c }}>
              {x.s}{mask(x.v)}
            </div>
          </div>
        ))}
      </div>
      <div style={{ marginTop: 12, background: '#FFFFFF', borderRadius: 14,
        border: '1px solid rgba(28,26,24,0.12)', overflow: 'hidden' }}>
        {flows.length === 0 && (
          <div style={{ padding: '32px 16px', textAlign: 'center', fontSize: 14,
            ...
            <div key={i} style={{
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: 'flex', alignItems: 'baseline', gap: 6 }}>
                  ...
                  {fresh && (
                    <span style={{ fontSize: 14, padding: '1px 6px', borderRadius: 20,
          ...
          return (
            <div key={i} style={{
              ...
              <div style={{ textAlign: 'right' }}>
                ...
                {t.pnl !== undefined && (
                  <div style={{ marginTop: 2, fontSize: 14, color: '#A8BD8C',
                    fontFamily: 'JetBrains Mono, monospace' }}>
                    已實現 +{mask(t.pnl)}
                  </div>
                )}
...
function DashWidget({ which, hideAmounts, accountsPie, stocks }) {
  ...
  if (which === 'spending') {
    ...
    Icon = Activity;
    const expCats = [
      { label: '餐飲',     amount: 15420, color: '#D97757' },
      { label: '交通',     amount: 10100, color: '#D4B87A' },
      { label: '生活雜貨', amount:  8240, color: '#A8BD8C' },
      { label: '娛樂',     amount:  6760, color: '#C5A07D' },
      { label: '其他',     amount:  7710, color: '#8FA86F' },
    ];
    const total = expCats.reduce((a, b) => a + b.amount, 0);
    pie = expCats.map(c => ({ label: c.label, color: c.color, pct: c.amount / total * 100, value: c.amount }));
    centerLabel = '本月支出';
    centerValue = mask(total);
    centerSub = 'NT$';
  ...
  return (
    <>
      <div style={{ marginTop: 22, display: 'flex', alignItems: 'center', gap: 8,
        color: 'rgba(18,17,12,0.72)', letterSpacing: 1, textTransform: 'uppercase', fontSize: 14 }}>
        <Icon size={14}/> {title}
      </div>
      <div style={{
        marginTop: 12, padding: '20px',
        background: '#FFFFFF', borderRadius: 20, border: '1px solid rgba(28,26,24,0.12)',
        ...
        <div style={{ position: 'relative', flexShrink: 0 }}>
          ...
          <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column',
            ...
            <div style={{ fontSize: 14, color: 'rgba(28,26,24,0.60)' }}>{centerLabel}</div>
            ...
            <div style={{ fontSize: 14, color: 'rgba(45,36,32,0.4)' }}>{centerSub}</div>
          </div>
        </div>
...
function DashboardScreen({ hideAmounts, setHideAmounts, savedFlows = [], savedTrades = [], dashWidget = 'accounts' }) {
  ...
  // Snapshot stock positions don't change by selected date
  const accounts = [
  ...
  const pie = [
    { label: '現金 / 帳戶', pct: Math.max(0, cashTotal) / total * 100, color: '#D97757' },
    { label: '台股持倉',     pct: stockTotal / total * 100, color: '#A8BD8C' },
  ];
  const mask = (v) => hideAmounts ? '••••••' : fmtMoney(Math.round(v));
    ...
    <div style={{ padding: '12px 18px 32px', color: '#18110C' }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12 }}>
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ marginTop: 6, display: 'flex', alignItems: 'center', gap: 6 }}>
              <div style={{
                ...
                fontSize: 14, fontWeight: 500, fontFamily: 'JetBrains Mono, monospace'
              }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexShrink: 0, marginTop: -4 }}>
            ...
            }}>{hideAmounts ? <EyeOff size={15}/> : <Eye size={15}/>}</button>
            <button onClick={doRefresh} disabled={refreshing} title={`更新 · ${refreshedAt}`} style={{
              ...
              color: '#E89878', display: 'flex', alignItems: 'center', justifyContent: 'center',
      ...
      <div
        ...
        <div key={dayKey(selectedDate)} style={{
          ...
          <DailyView date={selectedDate} hideAmounts={hideAmounts}
