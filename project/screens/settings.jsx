...
function SettingsScreen() {
  ...
  // Master data
  const [data, setData] = useStateSet(DEFAULT_DATA);
  ...
  const Section = ({ label, children }) => (
    <div style={{ marginTop: 22 }}>
      <div style={{
        fontSize: 14, color: 'rgba(18,17,12,0.72)', letterSpacing: 1,
  ...
  return (
    <div style={{ padding: '8px 18px 32px', color: '#18110C' }}>
      {/* Privacy hero */}
      <div style={{
        marginTop: 4, padding: '18px 20px', borderRadius: 20,
        background: '#18110C',
        ...
        <div style={{ marginTop: 12, display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8 }}>
          {[
            ...
            <div key={k} style={{
              ...
              <div style={{ fontSize: 14, fontWeight: 600, color: '#A8BD8C', marginTop: 1,
      ...
      <Section label="主檔管理">
        <ManageRow icon={<Tag size={18}/>} color="#D97757"
          label="記帳分類" count={data.cat_exp.length + data.cat_inc.length + data.cat_xfer.length}
          sub="支出 / 收入 / 轉帳 分類"
              ...
              {MODELS.map(m => (
                <button key={m.id} onClick={() => { setDefaultModel(m.id); setModelOpen(false); }}
                  ...
                  style={{
                    ...
                    fontSize: 15, fontWeight: m.id === defaultModel ? 600 : 500,
                  }}>
                  <Sparkles size={14}/>
...
function Row({ icon, iconColor, label, sub, detail, chevron }) {
  ...
  return (
    <button style={{
      ...
      {detail && <span style={{ fontSize: 14, color: 'rgba(18,17,12,0.72)', marginRight: 4,
...
function ManageRow({ icon, color, label, sub, count, onClick }) {
  ...
  return (
    <button onClick={onClick} style={{
      ...
      <span style={{
        padding: '3px 10px', borderRadius: 8, fontSize: 14, fontWeight: 600,
  ...
  return (
    <div style={{
      ...
      <button onClick={() => onChange(!value)} style={{
        width: 52, height: 32, borderRadius: 14, flexShrink: 0,
        ...
        <span style={{
          ...
          borderRadius: 14, background: '#fff', transition: 'left 200ms',
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
        ...
        {status === 'connected' && (
          <span style={{
            ...
            padding: '3px 8px', borderRadius: 8, fontSize: 14,
      ...
      <div style={{ marginTop: 10, display: 'flex', gap: 8, paddingLeft: 50 }}>
        <input
          ...
          style={{
            ...
            color: '#18110C', fontSize: 14, outline: 'none',
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: 8 }}>
        <button onClick={() => { setSortMode(s => !s); setEditingIdx(null); }} style={{
        ...
        {items.map((it, i) => (
          <div key={i} style={{
            ...
            {sortMode ? (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 2, flexShrink: 0 }}>
                <button onClick={() => move(i, -1)} disabled={i === 0} style={{
                  width: 30, height: 18, borderRadius: 8,
                ...
                <button onClick={() => move(i, 1)} disabled={i === items.length - 1} style={{
                  width: 30, height: 18, borderRadius: 8,
            ...
            {editingIdx === i ? (
              ...
              <button onClick={() => !sortMode && startEdit(i)} style={{
                flex: 1, minWidth: 0, height: 36, background: 'transparent', border: 'none',
                color: '#18110C', fontSize: 15, textAlign: 'left', padding: 0,
      ...
      <div style={{
        display: sortMode ? 'none' : 'flex', gap: 8, padding: 4, borderRadius: 14,
...
function AccountsManager({ data, onChange, color }) {
  const { Plus, X, Check, Wallet } = window.Icons;
  const [editingIdx, setEditingIdx] = useStateSet(null);
  ...
  return (
    <>
              ...
              <div style={{ marginTop: 8, display: 'flex', flexWrap: 'wrap', gap: 6, paddingLeft: 40 }}>
                {KINDS.map(k => (
                  <button key={k} onClick={() => setEdit({ ...edit, kind: k })} style={{
                    padding: '4px 10px', borderRadius: 8, fontSize: 14, fontWeight: 500,
      ...
      {adding ? (
        <div style={{
          padding: 10, borderRadius: 14, background: '#FFFFFF',
          ...
          <div style={{ marginTop: 10, display: 'flex', gap: 8 }}>
            <button onClick={() => { setAdding(false); setAddV({ name: '', kind: '銀行' }); }} style={{
              ...
              color: 'rgba(45,36,32,0.7)', fontSize: 14,
            }}>取消</button>
...
function KeyValueManager({ items, onChange, color, fields, placeholders }) {
  const { Plus, X, Check, ArrowUpRight } = window.Icons;
  const [editingIdx, setEditingIdx] = useStateSet(null);
            ...
            {editingIdx === i ? (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                <input autoFocus value={edit.name || ''}
                  ...
                  style={{
                    ...
                    background: 'rgba(0,0,0,0.08)', border: `1px solid ${color}55`,
                    color: '#18110C', fontSize: 15, fontWeight: 500, outline: 'none',
                  }}/>
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  ...
                  <button onClick={saveEdit} style={{
                    ...
                    border: 'none', color: '#1a1a1a', fontSize: 14, fontWeight: 600,
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                ...
                <button onClick={() => startEdit(i)} style={{
                  ...
                  {it.sub && (
                    <div style={{ fontSize: 14, color: 'rgba(45,36,32,0.5)', marginTop: 1 }}>{it.sub}</div>
                  )}
                </button>
                <button onClick={() => remove(i)} style={{
                  width: 36, height: 36, borderRadius: 8, flexShrink: 0,
