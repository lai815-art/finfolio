  ...
  const health = (() => {
    if (stockPct > 55) {
      return {
        ...
        node: <>目前股票資產佔比達 <b style={{ color: '#D88770' }}>{r0(stockPct)}%</b> 偏高。
    ...
    if (cashPct > 55) {
      return {
        ...
        node: <>現金部位高達 <b style={{ color: '#A8BD8C' }}>{r0(cashPct)}%</b>，
          長期恐被通膨侵蝕。股票僅 <b style={{ color: '#D88770' }}>{r1(stockPct)}%</b>、
          債券 <b style={{ color: '#BFA176' }}>{r1(bondPct)}%</b>，
          建議分批將閒置現金配置至 <b>投資等級債券 ETF 與定期定額台股</b>，提升資產效率。</>,
      };
    }
    return {
      score: 80, level: '良好',
      node: <>資產配置大致均衡：股票 <b style={{ color: '#D88770' }}>{r1(stockPct)}%</b>、
        債券 <b style={{ color: '#BFA176' }}>{r1(bondPct)}%</b>、
        現金 <b style={{ color: '#A8BD8C' }}>{r1(cashPct)}%</b>。維持紀律、定期再平衡即可。</>,
    };
  })();
  ...
  return (
    <div style={{ height: '100%', color: '#18110C', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{
        ...
        <div style={{ fontSize: 14, color: 'rgba(45,36,32,0.6)', lineHeight: 1.3,
      ...
      {/* Health check card */}
      <div style={{ marginTop: 22, fontSize: 15, color: 'rgba(18,17,12,0.72)', letterSpacing: 1,
        textTransform: 'uppercase', padding: '0 4px', display: 'flex', alignItems: 'center', gap: 8 }}>
        <Activity size={14}/> 資產配置健康檢查
      </div>
      <div style={{
        marginTop: 10, padding: '22px 22px', borderRadius: 26,
        background: '#FFFFFF',
        border: '1px solid rgba(28,26,24,0.12)',
      }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12 }}>
          <div style={{ fontSize: 15, color: '#D4B87A', fontWeight: 600, letterSpacing: 1,
            padding: '4px 10px', borderRadius: 8, background: 'rgba(212, 184, 122,0.12)',
            border: '1px solid rgba(212, 184, 122,0.3)', whiteSpace: 'nowrap', flexShrink: 0 }}>
            🔍 主動分析
          </div>
          <div style={{ fontSize: 15, color: 'rgba(45,36,32,0.4)', fontFamily: 'JetBrains Mono, monospace', whiteSpace: 'nowrap', flexShrink: 0 }}>
            05.27 14:30
          </div>
        </div>
        <div style={{ marginTop: 16, display: 'flex', alignItems: 'center', gap: 14 }}>
          <div style={{ position: 'relative', width: 64, height: 64, flexShrink: 0 }}>
            <svg width="64" height="64" viewBox="0 0 64 64" style={{ transform: 'rotate(-90deg)' }}>
              ...
              <circle cx="32" cy="32" r="27" fill="none" stroke="#A8BD8C" strokeWidth="6"
                strokeLinecap="round"
            ...
            <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column',
              ...
              <div style={{ fontSize: 19, fontWeight: 700, color: '#18110C',
                fontFamily: 'JetBrains Mono, monospace', lineHeight: 1 }}>{health.score}</div>
            </div>
          </div>
          <div>
            <div style={{ fontSize: 15, color: 'rgba(45,36,32,0.55)', letterSpacing: 1,
        ...
        <div style={{ marginTop: 16, display: 'flex', gap: 10 }}>
          <button style={{
            ...
            color: '#A8BD8C', fontSize: 16, fontWeight: 500,
          }}>查看完整報告</button>
          <button style={{
            flex: 1, height: 48, borderRadius: 18,
            background: 'rgba(28,26,24,0.12)', border: '1px solid rgba(28,26,24,0.16)',
            color: 'rgba(45,36,32,0.7)', fontSize: 16,
          }}>暫不調整</button>
        </div>
      </div>

      {/* Chat */}
      <div style={{ marginTop: 24, fontSize: 15, color: 'rgba(18,17,12,0.72)', letterSpacing: 1,
        textTransform: 'uppercase', padding: '0 4px', display: 'flex', alignItems: 'center', gap: 8 }}>
      ...
      <div style={{ marginTop: 10, display: 'flex', flexWrap: 'wrap', gap: 8 }}>
        {suggestions.map(s => (
          <button key={s} onClick={() => send(s)} style={{
            padding: '10px 14px', minHeight: 44, borderRadius: 18,
            background: 'rgba(217, 119, 87,0.08)',
            border: '1px solid rgba(217, 119, 87,0.22)',
            color: '#E89878', fontSize: 15, fontWeight: 500,
          }}>{s}</button>
        ))}
      </div>
      ...
      {/* Chat messages */}
      <div style={{
        marginTop: 14, padding: 14, borderRadius: 26,
        ...
        {chat.map((m, i) => (
          <div key={i} style={{
            ...
            <div style={{
              ...
              fontSize: 16, lineHeight: 1.55, letterSpacing: 0.1,
        ...
        {aiTyping && (
          <div style={{ display: 'flex', gap: 10, alignItems: 'flex-end' }}>
            ...
            <div style={{ padding: '14px 16px', borderRadius: 18, background: 'rgba(28,26,24,0.12)',
              display: 'flex', gap: 5, alignItems: 'center' }}>
              {[0,1,2].map(i => (
                <span key={i} style={{ width: 7, height: 7, borderRadius: 26,
      ...
      <div style={{ padding: '10px 18px 12px', background: '#F7F2EC',
        borderTop: '1px solid rgba(28,26,24,0.12)' }}>
      {/* Input bar */}
      <div style={{ marginTop: 12, display: 'flex', gap: 10, alignItems: 'center' }}>
        <input value={input} onChange={e => setInput(e.target.value)}
          ...
          style={{
            flex: 1, height: 56, padding: '0 18px', borderRadius: 18,
            background: '#FFFFFF', border: '1px solid rgba(28,26,24,0.14)',
            color: '#18110C', fontSize: 17, outline: 'none',
          }}/>
