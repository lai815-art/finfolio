...
function AdvisorScreen() {
  ...
  const aiReply = (q) => {
    ...
    setTimeout(() => {
      ...
      else if (q.includes('安全') || q.includes('股票')) reply = '台股佔比 60%，集中於半導體類股。短期波動風險偏高，但長期持有成本良好。若擔心回檔，可保留 3-6 個月生活費的現金緩衝。';
  ...
  return (
    <div style={{ height: '100%', color: '#18110C', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{
        ...
        <div style={{ fontSize: 12, color: 'rgba(45,36,32,0.6)', lineHeight: 1.3,
      ...
      {/* Health check card */}
      <div style={{ marginTop: 22, fontSize: 14, color: 'rgba(18,17,12,0.72)', letterSpacing: 1,
        textTransform: 'uppercase', padding: '0 4px', display: 'flex', alignItems: 'center', gap: 8 }}>
        <Activity size={14}/> 資產配置健康檢查
      </div>
      <div style={{
        marginTop: 10, padding: '22px 22px', borderRadius: 20,
        background: '#FFFFFF',
        border: '1px solid rgba(28,26,24,0.12)',
      }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12 }}>
          <div style={{ fontSize: 14, color: '#D4B87A', fontWeight: 600, letterSpacing: 1,
          ...
          <div style={{ fontSize: 14, color: 'rgba(45,36,32,0.4)', fontFamily: 'JetBrains Mono, monospace', whiteSpace: 'nowrap', flexShrink: 0 }}>
            05.27 14:30
          </div>
        </div>
        <div style={{ marginTop: 14, fontSize: 19, fontWeight: 500, color: '#18110C', lineHeight: 1.55, letterSpacing: 0.2 }}>
          目前股票資產佔比達 <b style={{ color: '#D88770' }}>60%</b> 偏高。
        ...
        <div style={{ marginTop: 16, display: 'flex', gap: 10 }}>
          <button style={{
            ...
            color: '#A8BD8C', fontSize: 15, fontWeight: 500,
          }}>查看完整報告</button>
          <button style={{
            flex: 1, height: 48, borderRadius: 14,
      ...
      {/* Chat */}
      <div style={{ marginTop: 24, fontSize: 14, color: 'rgba(18,17,12,0.72)', letterSpacing: 1,
        textTransform: 'uppercase', padding: '0 4px', display: 'flex', alignItems: 'center', gap: 8 }}>
      ...
      <div style={{ marginTop: 10, display: 'flex', flexWrap: 'wrap', gap: 8 }}>
        {suggestions.map(s => (
          <button key={s} onClick={() => send(s)} style={{
            padding: '10px 14px', minHeight: 44, borderRadius: 14,
            background: 'rgba(217, 119, 87,0.08)',
            border: '1px solid rgba(217, 119, 87,0.22)',
            color: '#E89878', fontSize: 14, fontWeight: 500,
          }}>{s}</button>
        ))}
      </div>
      ...
      {/* Chat messages */}
      <div style={{
        marginTop: 14, padding: 14, borderRadius: 20,
        ...
        {aiTyping && (
          <div style={{ display: 'flex', gap: 10, alignItems: 'flex-end' }}>
            ...
            <div style={{ padding: '14px 16px', borderRadius: 14, background: 'rgba(28,26,24,0.12)',
              display: 'flex', gap: 5, alignItems: 'center' }}>
              {[0,1,2].map(i => (
                <span key={i} style={{ width: 7, height: 7, borderRadius: 20,
