  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
      ...
      <button onClick={() => setOpen(true)} style={{
        flex: 1, minWidth: 0, height: 60, padding: '0 12px', borderRadius: 14,
        background: '#FFFFFF', border: '1px solid rgba(217,119,87,0.32)',
        color: '#F2B89C',
      ...
      <button onClick={() => step(1)} disabled={isToday} style={{
        width: 60, height: 60, borderRadius: 14, flexShrink: 0,
        background: '#FFFFFF', border: '1px solid rgba(28,26,24,0.12)',
...
function AccountingScreen({ onSaved, autoVoice }) {
  const [mode, setMode] = useStateAcc('flow'); // 'flow' | 'stock'

  // Flow form state (lifted)
  const [flow, setFlow] = useStateAcc({
    kind: 'exp', amount: '', category: '餐飲',
    account: '信用卡 A', fromAccount: '主要存款帳戶', toAccount: '券商交割戶',
    date: new Date(TODAY_ACC), note: '',
  });
  const updateFlow = (patch) => setFlow(f => ({ ...f, ...patch }));

  // Stock form state (lifted)
  const [stock, setStock] = useStateAcc({
    side: 'buy', code: '', name: '', shares: '', price: '',
    assetClass: '股票', broker: '主要券商', settleAccount: '券商交割戶',
    date: new Date(TODAY_ACC), note: '',
  });
  const updateStock = (patch) => setStock(s => ({ ...s, ...patch }));

  // Unified voice
  const [voiceState, setVoiceState] = useStateAcc('idle'); // idle, listening, done
  const [voiceText, setVoiceText]   = useStateAcc('');
  const [voiceResult, setVoiceResult] = useStateAcc(null);
  const [voiceTurn, setVoiceTurn]   = useStateAcc(0);

  const startVoice = () => {
    const sc = VOICE_SCENARIOS[voiceTurn % VOICE_SCENARIOS.length];
    setVoiceTurn(t => t + 1);
    ...
    const id = setInterval(() => {
      ...
      if (i >= sc.text.length) {
        ...
        setTimeout(() => {
          if (sc.intent === 'flow') {
            setMode('flow'); updateFlow(sc.apply);
          } else {
            setMode('stock'); updateStock(sc.apply);
          }
          setVoiceResult(sc);
          setVoiceState('done');
        }, 500);
  ...
  const resetVoice = () => { setVoiceState('idle'); setVoiceText(''); setVoiceResult(null); };
  ...
  return (
    <div style={{ paddingTop: 4 }}>
      ...
      <div style={{ padding: '0 18px' }}>
        <UnifiedVoice
          state={voiceState} text={voiceText} result={voiceResult}
          onStart={startVoice} onReset={resetVoice}/>
      </div>
        ...
        <div style={{
          ...
          {[
            ...
            <button key={t.id} onClick={() => setMode(t.id)} style={{
              ...
              fontSize: 15, fontWeight: mode === t.id ? 600 : 500,
            }}>{t.label}</button>
          ))}
        </div>
      </div>
  ...
  return (
    <div style={{
      ...
      background: state !== 'idle'
        ? 'linear-gradient(155deg, #F0E9E2 0%, #FFFFFF 100%)'
        : '#FFFFFF',
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
        <button onClick={state === 'idle' ? onStart : onReset} style={{
          ...
          display: 'flex', alignItems: 'center', justifyContent: 'center',
          boxShadow: '0 10px 22px rgba(217,119,87,0.45)', position: 'relative',
          cursor: 'pointer',
        }}>
          {state === 'listening' && (
            <>
              <span style={{ position: 'absolute', inset: -10, borderRadius: 46,
                border: '2px solid rgba(217,119,87,0.5)',
                animation: 'pulse 1.4s ease-out infinite' }}/>
              <span style={{ position: 'absolute', inset: -4, borderRadius: 40,
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 14,
            color: '#E89878', letterSpacing: 0.5 }}>
      ...
      {text && (
        <div style={{
          ...
          background: 'rgba(45,36,32,0.05)', border: '1px solid rgba(28,26,24,0.12)',
          fontSize: 15, lineHeight: 1.5, color: '#18110C',
        }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 14,
            color: 'rgba(28,26,24,0.60)', letterSpacing: 1,
            textTransform: 'uppercase', marginBottom: 6 }}>
            <Volume size={12}/> 語音轉文字
          </div>
          {text}
          {state === 'listening' && (
            <span style={{ display: 'inline-block', width: 2, height: 15, background: '#E89878',
      ...
      {result && state === 'done' && (
        <div style={{
          ...
          <span style={{ fontSize: 14, color: '#A8BD8C', display: 'inline-flex',
            alignItems: 'center', gap: 4 }}>
            <Sparkles size={12}/> 解析
          </span>
          {result.summary.map(([k, v]) => (
            <span key={k} style={{
              fontSize: 14, padding: '3px 8px', borderRadius: 8,
              background: 'rgba(28,26,24,0.12)', color: '#18110C',
  ...
  return (
    <div style={{ position: 'relative' }}>
      <button onClick={() => setOpen(!open)} style={{
        width: '100%', minHeight: 60, padding: '8px 14px', borderRadius: 14,
        background: '#FFFFFF', border: open ? '1px solid rgba(217,119,87,0.45)' : '1px solid rgba(28,26,24,0.12)',
        color: '#18110C', display: 'flex', alignItems: 'center', gap: 10, textAlign: 'left',
      }}>
        {icon && <span style={{ color: 'rgba(45,36,32,0.5)', flexShrink: 0 }}>{icon}</span>}
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: 14, color: 'rgba(45,36,32,0.5)', letterSpacing: 0.5,
            textTransform: 'uppercase' }}>{label}</div>
          <div style={{ marginTop: 1, fontSize: 16, fontWeight: 500,
            overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{value}</div>
        </div>
        <ChevronDown size={16} style={{ color: 'rgba(45,36,32,0.5)', flexShrink: 0,
          transition: 'transform 200ms', transform: open ? 'rotate(180deg)' : 'none' }}/>
      </button>
      {open && (
        <div style={{
          position: 'absolute', top: 'calc(100% + 6px)', left: 0, right: 0, zIndex: 30,
          background: '#FBF7F2', borderRadius: 14, padding: 6,
          ...
          {options.map(o => (
            <button key={o} onClick={() => { onChange(o); setOpen(false); }}
              style={{
                ...
                color: o === value ? '#FFFFFF' : '#18110C', fontSize: 15,
...
function SectionLabel({ children, action }) {
  return (
    <div style={{ marginTop: 22, marginBottom: 10, padding: '0 4px',
      display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
      <div style={{ fontSize: 14, color: 'rgba(18,17,12,0.72)', letterSpacing: 1,
...
/* ============= 收支轉帳 form ============= */
function FlowForm({ state, update, onSaved }) {
  const { Plus, Tag, CreditCard, Calendar, ArrowRight, Wallet } = window.Icons;
  ...
  return (
    <div style={{ padding: '8px 18px 28px', color: '#18110C' }}>
        ...
        {KINDS.map(k => {
          ...
          return (
            <button key={k.id} onClick={() => {
              ...
              fontSize: 16, fontWeight: on ? 700 : 500, gap: 2,
              boxShadow: 'none',
            }}>
              <span style={{
                fontSize: 14, fontFamily: 'JetBrains Mono, monospace',
                letterSpacing: 1, opacity: on ? 0.7 : 0.55,
      ...
      <div style={{
        padding: '16px 20px', borderRadius: 14, background: '#FFFFFF',
        border: '1px solid rgba(28,26,24,0.12)',
      }}>
        <div style={{ display: 'flex', alignItems: 'baseline', gap: 8 }}>
          <span style={{ fontSize: 20, color: 'rgba(18,17,12,0.72)',
            fontFamily: 'JetBrains Mono, monospace' }}>NT$</span>
          <input value={state.amount} onChange={e => update({ amount: e.target.value })}
            placeholder="0" inputMode="decimal"
            style={{
              flex: 1, background: 'transparent', border: 'none', outline: 'none',
              fontSize: 40, fontWeight: 600, color: active.color,
      ...
      {state.kind === 'xfer' ? (
        <>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <div style={{
              width: 32, height: 32, borderRadius: 14, flexShrink: 0,
              background: 'rgba(197,160,125,0.18)', border: '1px solid rgba(197,160,125,0.4)',
              color: '#C5A07D', display: 'flex', alignItems: 'center', justifyContent: 'center',
      ...
      <div style={{ padding: '14px 18px', borderRadius: 14, background: '#FFFFFF',
        border: '1px solid rgba(28,26,24,0.12)' }}>
        <input value={state.note} onChange={e => update({ note: e.target.value })}
          placeholder="輸入備註…"
          style={{
            width: '100%', background: 'transparent', border: 'none', outline: 'none',
            fontSize: 16, color: '#18110C',
          }}/>
      </div>
      ...
      <button onClick={() => {
        ...
        marginTop: 18, width: '100%', height: 60, borderRadius: 14,
        background: `linear-gradient(135deg, ${active.color}, ${active.color}cc)`,
        border: 'none', color: '#1a1a1a', fontSize: 17, fontWeight: 600,
        display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8,
...
/* ============= 股票買賣 form ============= */
function StockForm({ state, update, onSaved }) {
  const { Plus, TrendUp, TrendDown, Search, Calendar } = window.Icons;
  ...
  const matches = showMatches
    ? universe.filter(u => u.code.startsWith(state.code) || u.name.includes(state.code))
    : [];

  const pick = (s) => update({ code: s.code, name: s.name, price: String(s.last), assetClass: s.class });
  ...
  // Available brokers / settle accounts depend on asset class
  ...
  return (
    <div style={{ padding: '8px 18px 28px', color: '#18110C' }}>
        ...
        {SIDES.map(s => {
          ...
          return (
            <button key={s.id} onClick={() => update({ side: s.id })} style={{
              ...
              color: on ? '#FFFFFF' : 'rgba(45,36,32,0.6)',
              display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8,
              fontSize: 17, fontWeight: on ? 700 : 500,
            }}>
      ...
      <div style={{
        padding: '4px 14px', borderRadius: 14, background: '#FFFFFF',
        border: '1px solid rgba(28,26,24,0.12)',
        ...
        <input value={state.code} onChange={e => update({ code: e.target.value.toUpperCase(), name: '' })}
          placeholder="代號" style={{
            ...
            fontSize: 20, fontWeight: 600, color: '#18110C',
            fontFamily: 'JetBrains Mono, monospace',
          }}/>
        <input value={state.name} onChange={e => update({ name: e.target.value })}
          ...
          style={{
            ...
            fontSize: 16, color: 'rgba(45,36,32,0.85)',
        ...
        <div style={{ marginTop: 8, display: 'flex', flexDirection: 'column', gap: 6 }}>
          {matches.slice(0, 4).map(s => (
            <button key={s.code} onClick={() => pick(s)} style={{
              ...
              border: '1px solid rgba(28,26,24,0.12)', color: '#18110C',
              textAlign: 'left',
            }}>
              ...
              <span style={{ fontSize: 14, padding: '2px 8px', borderRadius: 5,
                background: 'rgba(28,26,24,0.12)', color: 'rgba(45,36,32,0.6)' }}>
                {s.class}
              </span>
              <span style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 14,
                color: 'rgba(18,17,12,0.72)' }}>{s.last}</span>
            </button>
          ))}
        </div>
      )}
        ...
        {[
          ...
          <div key={f.k} style={{
            ...
            <input value={state[f.k]} onChange={e => update({ [f.k]: e.target.value })}
              ...
              style={{
                marginTop: 4, width: '100%', background: 'transparent', border: 'none', outline: 'none',
                fontSize: 24, fontWeight: 600, color: '#18110C',
                fontFamily: 'JetBrains Mono, monospace',
              }}/>
          </div>
        ))}
      ...
      <div style={{ marginTop: 10, display: 'flex', gap: 6 }}>
        {[100, 500, 1000].map(n => (
          <button key={n} onClick={() => update({ shares: String(n) })} style={{
            ...
            color: 'rgba(45,36,32,0.7)', fontFamily: 'JetBrains Mono, monospace',
            fontSize: 14, fontWeight: 500,
          }}>{n.toLocaleString()} 股</button>
        ))}
      </div>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 14,
          ...
          <span>成交金額</span>
          <span style={{ fontFamily: 'JetBrains Mono, monospace', color: '#18110C' }}>
            NT$ {gross.toLocaleString(undefined, { maximumFractionDigits: 0 })}
          </span>
        </div>
        <div style={{ marginTop: 4, display: 'flex', justifyContent: 'space-between', fontSize: 14,
          color: 'rgba(45,36,32,0.6)' }}>
        ...
        <div style={{ marginTop: 10, paddingTop: 10,
          ...
          <span style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 22, fontWeight: 700,
            color: accent }}>
            NT$ {net.toLocaleString(undefined, { maximumFractionDigits: 0 })}
          </span>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8 }}>
        ...
        <DropField label="券商" value={state.broker}
          options={brokers} onChange={v => update({ broker: v })}/>
