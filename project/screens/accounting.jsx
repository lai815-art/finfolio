  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
      ...
      <button onClick={() => setOpen(true)} style={{
        flex: 1, minWidth: 0, height: 60, padding: '0 12px', borderRadius: 18,
...
function AccountingScreen({ onSaved, onDelete, initialDraft, masterData }) {
        ...
        <div style={{
          ...
          {[
            ...
            <button key={t.id} onClick={() => setMode(t.id)} style={{
              ...
              fontSize: 16, fontWeight: mode === t.id ? 600 : 500,
            }}>{t.label}</button>
          ))}
        </div>
      </div>
  ...
  return (
    <div style={{ padding: '8px 18px 0' }}>
      <div style={{
        ...
        background: 'linear-gradient(155deg, #F0E9E2 0%, #FFFFFF 100%)',
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <div style={{
            ...
            color: '#7C9A5A', display: 'flex', alignItems: 'center', justifyContent: 'center',
          }}><Sparkles size={15}/></div>
          <div style={{ fontSize: 15, fontWeight: 600, color: '#18110C' }}>AI 已帶入，請確認或補充</div>
        </div>
        <div style={{
          marginTop: 10, padding: '8px 12px', borderRadius: 10,
          background: 'rgba(28,26,24,0.05)', display: 'flex', alignItems: 'flex-start', gap: 6,
          fontSize: 15, color: 'rgba(45,36,32,0.7)', lineHeight: 1.4,
        }}>
        ...
        <div style={{ marginTop: 8, display: 'flex', flexWrap: 'wrap', gap: 6 }}>
          {draft.summary.map(([k, v]) => (
            <span key={k} style={{
              ...
              background: 'rgba(168,189,140,0.16)', color: '#5F7A44',
...
function UnifiedVoice({ state, text, result, onStart, onReset }) {
  ...
  return (
    <div style={{
      padding: '18px 20px', borderRadius: 26,
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ marginTop: 2, fontSize: 15, color: 'rgba(45,36,32,0.5)' }}>
            支援 收支 / 轉帳 / 股票買賣 / 債券
          </div>
        </div>
      </div>
      ...
      {text && (
        <div style={{
          marginTop: 14, padding: '12px 14px', borderRadius: 18,
          background: 'rgba(45,36,32,0.05)', border: '1px solid rgba(28,26,24,0.12)',
          fontSize: 16, lineHeight: 1.5, color: '#18110C',
        }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 15,
      ...
      {result && state === 'done' && (
        <div style={{
          ...
          <span style={{ fontSize: 15, color: '#A8BD8C', display: 'inline-flex',
            alignItems: 'center', gap: 4 }}>
            <Sparkles size={12}/> 解析
          </span>
          {result.summary.map(([k, v]) => (
            <span key={k} style={{
              fontSize: 15, padding: '3px 8px', borderRadius: 8,
  ...
  return (
    <div style={{ position: 'relative' }}>
      <button onClick={() => setOpen(!open)} style={{
        width: '100%', minHeight: 60, padding: '8px 14px', borderRadius: 18,
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: 15, color: 'rgba(45,36,32,0.5)', letterSpacing: 0.5,
            textTransform: 'uppercase' }}>{label}</div>
          <div style={{ marginTop: 1, fontSize: 17, fontWeight: 500,
      ...
      {open && (
        <div style={{
          ...
          background: '#FBF7F2', borderRadius: 18, padding: 6,
          ...
          {options.map(o => (
            <button key={o} onClick={() => { onChange(o); setOpen(false); }}
              style={{
                ...
                color: o === value ? '#FFFFFF' : '#18110C', fontSize: 16,
...
function SectionLabel({ children, action }) {
  return (
    <div style={{ marginTop: 22, marginBottom: 10, padding: '0 4px',
      ...
      <div style={{ fontSize: 15, color: 'rgba(18,17,12,0.72)', letterSpacing: 1,
...
function FlowForm({ state, update, onSaved, onDelete, recordId, masterData }) {
  ...
  return (
    <div style={{ padding: '8px 18px 28px', color: '#18110C' }}>
        ...
        {KINDS.map(k => {
          ...
          return (
            <button key={k.id} onClick={() => {
              ...
              fontSize: 17, fontWeight: on ? 700 : 500, gap: 2,
              boxShadow: 'none',
            }}>
              <span style={{
      ...
      <div style={{
        padding: '16px 20px', borderRadius: 18, background: '#FFFFFF',
      ...
      {state.kind === 'xfer' ? (
        <>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <div style={{
              width: 32, height: 32, borderRadius: 18, flexShrink: 0,
      ...
      <div style={{ padding: '14px 18px', borderRadius: 18, background: '#FFFFFF',
        border: '1px solid rgba(28,26,24,0.12)' }}>
        <input value={state.note} onChange={e => update({ note: e.target.value })}
          placeholder="輸入備註…"
          style={{
            width: '100%', background: 'transparent', border: 'none', outline: 'none',
            fontSize: 17, color: '#18110C',
          }}/>
      </div>
      ...
      <div style={{ marginTop: 18, display: 'flex', gap: 10 }}>
        {recordId && (
          <button onClick={() => onDelete && onDelete(recordId)} style={{
            flex: '0 0 auto', padding: '0 22px', height: 60, borderRadius: 18,
            background: 'transparent', border: '1px solid rgba(216,135,112,0.4)',
            color: '#D88770', fontSize: 17, fontWeight: 600,
            display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8,
          }}>
        ...
        <button onClick={() => {
          ...
          border: 'none', color: '#1a1a1a', fontSize: 18, fontWeight: 600,
          display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8,
...
/* ============= 股票買賣 form ============= */
function StockForm({ state, update, onSaved, onDelete, recordId, masterData }) {
  ...
  return (
    <div style={{ padding: '8px 18px 28px', color: '#18110C' }}>
        ...
        {SIDES.map(s => {
          ...
          return (
            <button key={s.id} onClick={() => update({ side: s.id })} style={{
              ...
              color: on ? '#FFFFFF' : 'rgba(45,36,32,0.6)',
              display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8,
              fontSize: 18, fontWeight: on ? 700 : 500,
            }}>
      ...
      <div style={{
        padding: '4px 14px', borderRadius: 18, background: '#FFFFFF',
        border: '1px solid rgba(28,26,24,0.12)',
        ...
        <input value={state.name} onChange={e => update({ name: e.target.value })}
          ...
          style={{
            ...
            fontSize: 17, color: 'rgba(45,36,32,0.85)',
        ...
        <div style={{ marginTop: 8, display: 'flex', flexDirection: 'column', gap: 6 }}>
          {matches.slice(0, 4).map(s => (
            <button key={s.code} onClick={() => pick(s)} style={{
              ...
              <span style={{ fontSize: 15, padding: '2px 8px', borderRadius: 5,
      ...
      {/* Calc */}
      <div style={{
        marginTop: 16, padding: '14px 18px', borderRadius: 18,
        ...
        <div style={{ marginTop: 4, display: 'flex', justifyContent: 'space-between', fontSize: 15,
          color: 'rgba(45,36,32,0.6)' }}>
      ...
      <div style={{ marginTop: 18, display: 'flex', gap: 10 }}>
        ...
        <button onClick={() => {
          ...
          onSaved && onSaved('stock', { ...state, recordId });
        }} style={{
          flex: 1, height: 60, borderRadius: 18,
