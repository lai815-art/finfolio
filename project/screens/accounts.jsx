...
/* ─── Hero Carousel ──────────────────────────────────────────────────── */
function HeroCarousel({ netWorth, pieLight, investPie, mask }) {
  const [idx, setIdx] = useStateAcct(0);
  const touch = useRefAcct({ x: 0, active: false });
  const slides = ['networth', 'invest', 'annual'];
  const go = n => setIdx(Math.max(0, Math.min(slides.length - 1, n)));
  const onStart = e => { const p = e.touches ? e.touches[0] : e; touch.current = { x: p.clientX, active: true }; };
  const onEnd = e => {
  ...
  const annual = [
    { m: '1月', inc: 78, exp: 52 }, { m: '2月', inc: 82, exp: 60 },
    { m: '3月', inc: 80, exp: 48 }, { m: '4月', inc: 95, exp: 55 },
    { m: '5月', inc: 88, exp: 50 }, { m: '6月', inc: 92, exp: 64 },
  ];
  ...
  return (
    <div onTouchStart={onStart} onTouchEnd={onEnd} onMouseDown={onStart} onMouseUp={onEnd}
      ...
      <div style={{
        position: 'relative', overflow: 'hidden', padding: '18px 20px', borderRadius: 28, minHeight: 210,
        ...
        {slides[idx] === 'networth' && (
          <>
            ...
            <div style={{ position: 'relative', marginTop: 4, fontFamily: 'JetBrains Mono, monospace', fontSize: 28, fontWeight: 700, letterSpacing: -0.5, color: '#FFFFFF' }}>
              <span style={{ fontSize: 16, color: 'rgba(255,255,255,0.82)', marginRight: 4 }}>NT$</span>{mask(netWorth)}
            </div>
            <div style={{ position: 'relative', marginTop: 14, display: 'flex', alignItems: 'center', gap: 16 }}>
              <div style={{ position: 'relative', flexShrink: 0 }}>
                <PieDonutA data={pieLight} size={120} thickness={17}/>
                ...
                {pieLight.map(p => (
                  <div key={p.key} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 14, color: 'rgba(255,255,255,0.90)' }}>
                      <span style={{ width: 7, height: 7, borderRadius: 26, background: p.color }}/>{p.label}
                    </div>
        ...
        {slides[idx] === 'invest' && (
          <>
            ...
            <div style={{ position: 'relative', marginTop: 14, display: 'flex', alignItems: 'center', gap: 12 }}>
              <div style={{ position: 'relative', flexShrink: 0 }}>
                <PieDonutA data={investPie} size={116} thickness={16}/>
                <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
        ...
        {slides[idx] === 'annual' && (
          <>
            ...
            <div style={{ position: 'relative', marginTop: 10, display: 'flex', gap: 14, fontSize: 13, color: 'rgba(255,255,255,0.85)' }}>
              <span style={{ display: 'flex', alignItems: 'center', gap: 5 }}><span style={{ width: 8, height: 8, borderRadius: 2, background: '#FFFFFF' }}/>收入</span>
              <span style={{ display: 'flex', alignItems: 'center', gap: 5 }}><span style={{ width: 8, height: 8, borderRadius: 2, background: 'rgba(255,255,255,0.40)' }}/>支出</span>
            </div>
          </>
        )}
      </div>
      <div style={{ display: 'flex', justifyContent: 'center', gap: 7, marginTop: 12 }}>
        {slides.map((s, i) => (
          <button key={s} onClick={() => go(i)} style={{
            width: i === idx ? 22 : 8, height: 8, borderRadius: 8, padding: 0, border: 'none',
            background: i === idx ? '#C2562F' : 'rgba(28,26,24,0.18)', transition: 'all 200ms', cursor: 'pointer',
          }}/>
        ))}
      </div>
    </div>
...
function AccountDetailSheet({ data, mask, onClose, onSaveItem, savedFlows = [], savedTrades = [] }) {
  ...
  const realFlows = savedFlows
    ...
    .map(f => ({
      ...
      amount: f.kind === 'inc' ? f.amount : f.kind === 'xfer' ? 0 : -f.amount,
      source: 'real',
    }))
    ...
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '4px 16px 12px' }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: 19, fontWeight: 700, color: '#18110C',
      ...
      <div style={{ flex: 1, overflowY: 'auto', overflowX: 'hidden', padding: '0 18px 28px' }}>
        ...
        <div style={{
          ...
          <div style={{ position: 'relative' }}>
            <div style={{ fontSize: 13, color: 'rgba(255,255,255,0.78)', letterSpacing: 1, textTransform: 'uppercase' }}>
              {isCredit ? '本期帳款' : '目前餘額'}
            </div>
            ...
            {isCredit && item.extra && (
              <div style={{ marginTop: 12 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between',
                  fontSize: 13, color: 'rgba(255,255,255,0.75)', marginBottom: 6 }}>
                  <span>使用 {item.extra.used}%</span>
                  <span>可用 NT$ {mask(item.extra.limit - item.amount)}</span>
                </div>
            ...
            {isBrokerage && (
              <div style={{ marginTop: 10, padding: '8px 12px', borderRadius: 10,
                background: 'rgba(255,255,255,0.15)', border: '1px solid rgba(255,255,255,0.22)',
                fontSize: 14, color: 'rgba(255,255,255,0.88)', display: 'flex', alignItems: 'center', gap: 6 }}>
        ...
        {(hasReal || mockTxns.length > 0) && (() => {
          const displayList = hasReal
            ? allReal.map((r, i) => [r.date, r.desc, r.amount, i])
            : mockTxns.map(([d, desc, amt], i) => [d, desc, amt, i]);
          ...
          return (
            <>
              <div style={{ fontSize: 12, color: 'rgba(28,26,24,0.48)', letterSpacing: 1,
                ...
                {sectionLabel}
                {isMock && <span style={{ fontSize: 11, color: 'rgba(28,26,24,0.30)',
                  textTransform: 'none', letterSpacing: 0, background: 'rgba(28,26,24,0.07)',
                  padding: '1px 6px', borderRadius: 4 }}>示範資料</span>}
                {!isMock && <span style={{ fontSize: 11, color: 'rgba(28,26,24,0.35)',
              ...
              <div style={{ background: '#FFFFFF', borderRadius: 18,
                ...
                {displayList.map(([date, desc, amt, origIdx]) => {
                  ...
                  return (
                    <div key={origIdx} style={{ borderBottom: origIdx < displayList.length - 1 ? '1px solid rgba(28,26,24,0.09)' : 'none' }}>
                      <div onClick={() => !isActive && openEdit(origIdx, displayDesc, displayAmt)}
                        style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '13px 16px',
                          cursor: 'pointer', background: isActive ? 'rgba(217,119,87,0.06)' : 'transparent' }}>
                        <div style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 13,
                          color: 'rgba(45,36,32,0.40)', flexShrink: 0, width: 42 }}>{date}</div>
                        <div style={{ flex: 1, fontSize: 15, color: '#18110C',
                            ...
                            <div style={{ flex: 1 }}>
                              ...
                              <input value={editDesc} onChange={e => setEditDesc(e.target.value)}
                                style={{ width: '100%', height: 38, padding: '0 10px', borderRadius: 10,
                                  ...
                                  fontSize: 15, color: '#18110C', outline: 'none' }}/>
                            </div>
                            {!isZero && (
                              <div style={{ width: 110 }}>
        ...
        {/* Cash note */}
        {isCash && (
          <div style={{ marginTop: 8, padding: '12px 16px', borderRadius: 16,
            background: 'rgba(168,189,140,0.08)', border: '1px solid rgba(168,189,140,0.20)',
            fontSize: 14, color: 'rgba(45,36,32,0.58)', lineHeight: 1.6 }}>
  ...
  return (
    <div style={{ background: '#FFFFFF', borderRadius: 20, border: '1px solid rgba(28,26,24,0.12)', overflow: 'hidden' }}>
      <button onClick={() => setOpenId(open ? null : group.id)} style={{
        ...
        color: '#18110C', display: 'flex', alignItems: 'center', gap: 14, textAlign: 'left', minHeight: 70,
      }}>
        <div style={{
        ...
        <div style={{ textAlign: 'right', marginRight: 4 }}>
          <div style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 18, fontWeight: 600,
            color: group.sign < 0 ? '#D88770' : '#18110C' }}>
            {group.sign < 0 ? '-' : ''}{mask(sum)}
          </div>
        </div>
        <ChevronDown size={18} style={{ color: 'rgba(45,36,32,0.35)', flexShrink: 0,
      ...
      {open && (
        <div style={{ padding: '0 12px 12px' }}>
          <div style={{ background: 'rgba(45,36,32,0.04)', borderRadius: 14,
            border: '1px solid rgba(28,26,24,0.10)', overflow: 'hidden' }}>
...
function AccountItemRow({ item, group, mask, last, onOpen }) {
  ...
  return (
    <div onClick={onOpen} style={{
      ...
      borderBottom: last ? 'none' : '1px solid rgba(28,26,24,0.09)',
      minHeight: 56, transition: 'background 120ms',
    }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: 15, fontWeight: 500, color: '#18110C',
            overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{item.name}</div>
          {item.sub && <div style={{ fontSize: 12, color: 'rgba(28,26,24,0.45)', marginTop: 1 }}>{item.sub}</div>}
        </div>
        <div style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 15, fontWeight: 600, flexShrink: 0,
          color: group.sign < 0 ? '#D88770' : '#18110C' }}>
          {group.sign < 0 ? '-' : ''}{mask(item.amount)}
        </div>
      ...
      {isCredit && item.extra && (
        <div style={{ marginTop: 7, marginLeft: 46 }}>
          <div style={{ height: 4, borderRadius: 8, background: 'rgba(28,26,24,0.10)', overflow: 'hidden' }}>
            <div style={{ height: '100%', borderRadius: 8, width: `${item.extra.used}%`,
              background: item.extra.used > 70 ? '#D88770' : '#D4B87A' }}/>
          </div>
        </div>
      )}
    </div>
...
/* ─── Main AccountsScreen ────────────────────────────────────────────── */
function AccountsScreen({ hideAmounts, onOpenDetail, acctOverrides }) {
  ...
  const { pie } = window.computePortfolio();
  const LIGHT_TINTS = { cash: '#FFFFFF', stock: '#FFE3C2', bond: '#FFE0A3', other: '#FFD0B5' };
  const pieLight = pie.map(p => ({ ...p, color: LIGHT_TINTS[p.key] || '#FFFFFF' }));

  const INVEST_TINTS = ['#FFFFFF','#FFE3C2','#FFD0B5','#FFE0A3','#FFC9A8','#FFEED0','#FFD9B0'];
  const allInvest = (window.INVEST_HOLDINGS || []).flatMap(g => g.items);
  const investTotal = allInvest.reduce((a, x) => a + x.qty * x.price, 0);
  const investPie = allInvest.length === 0 ? [{ key: 'empty', label: '尚無持倉', color: '#E0DAD3', pct: 100 }] :
    allInvest.map((it, i) => ({
    key: it.code, label: `${it.code} ${it.name}`,
    color: INVEST_TINTS[i % INVEST_TINTS.length],
    pct: investTotal > 0 ? (it.qty * it.price) / investTotal * 100 : 0,
  }));

  // Apply overrides to ASSET_GROUPS
  const assetGroups = (window.ASSET_GROUPS || []).map(g => {
    const Icon = window.Icons[g.icon] || window.Icons.Wallet;
  ...
  // Allocation tip — only show when there's data
    ...
    <div style={{ padding: '8px 18px 32px', color: '#18110C' }}>
      ...
      <div style={{
        ...
        <div style={{ fontSize: 15, color: '#18110C', lineHeight: 1.55 }}>
          股票配置目前佔 <b style={{ color: '#D97757' }}>{stockPct.toFixed(0)}%</b>，
          債券僅 <b style={{ color: '#BFA176' }}>{bondPct.toFixed(1)}%</b>，
          建議增加<b>投資等級債券 ETF</b> 提升防禦力。
        </div>
      </div>
      )}
